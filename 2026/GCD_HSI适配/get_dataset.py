...
import pickle
import os
from data.hyperspectral import (
    get_indianpines_datasets,
    get_pavia_datasets,
    get_houston_datasets,
    get_salinas_datasets,
    get_salinasa_datasets,
    get_trento_datasets,
    subsample_classes as subsample_dataset_hsi
)
...
get_dataset_funcs = {
    ...
    'IndianPines': get_indianpines_datasets,
    'Pavia': get_pavia_datasets,
    'Houston': get_houston_datasets,
    'Salinas': get_salinas_datasets,
    'SalinasA': get_salinasa_datasets,
    'Trento': get_trento_datasets,
}
...
def get_class_splits(args):
    ...
    elif args.dataset_name == 'IndianPines':
        args.train_classes = range(10)  # 10个已知类
        args.unlabeled_classes = range(10, 16)  # 6个未知类（总共16类）
        args.image_size = 224

    elif args.dataset_name == 'Pavia':
        args.train_classes = range(6)  # 6个已知类
        args.unlabeled_classes = range(6, 9)  # 3个未知类（总共9类）
        args.image_size = 224

    elif args.dataset_name == 'Houston':
        args.train_classes = [0, 1, 2, 3, 4, 10, 11, 12, 13, 14]  # 10个已知类
        args.unlabeled_classes = [5, 6, 7, 8, 9]  # 5个未知类（总共15类）
        args.image_size = 224

    elif args.dataset_name == 'Salinas':
        args.train_classes = range(12)  # 12个已知类
        args.unlabeled_classes = range(12, 16)  # 4个未知类（总共16类）
        args.image_size = 224

    elif args.dataset_name == 'SalinasA':
        args.train_classes = range(3)  # 3个已知类
        args.unlabeled_classes = range(3, 6)  # 3个未知类（总共6类）
        args.image_size = 224
